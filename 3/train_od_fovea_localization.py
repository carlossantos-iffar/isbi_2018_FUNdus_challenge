import numpy as np
from model import localize_od_fv
import utils
import os
import argparse
...
model_out_dir = "models_branching_factor_" + str(FLAGS.branching_point)
...
img_out_dir = "localized_branching_factor_" + str(FLAGS.branching_point)
...
# create networks
network = localize_od_fv(img_size, FLAGS.branching_point) 
network.summary()
...
for epoch in range(n_epochs):
    ...
    if epoch in validation_epochs:
        ...
        Euclidean_disc, Euclidean_fovea = utils.mean_Euclidean(val_coords, val_predicted_coords)
        utils.print_metrics(epoch + 1, Euclidean_disc=Euclidean_disc, Euclidean_fovea=Euclidean_fovea)
            
        # save the weight
        network.save_weights(os.path.join(model_out_dir, "network_{}.h5".format(epoch + 1)))
        
        # save validation results
        utils.save_imgs_with_pts(val_fundus, val_predicted_coords, img_out_dir, epoch)
