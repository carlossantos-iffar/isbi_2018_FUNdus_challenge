...
model_out_dir = "/nfs/jaemin/isbi/2/models/dr/features_and_conv_{}_{}".format(FLAGS.data_ratio, FLAGS.loss_type)
...
if FLAGS.load_model_dir:
    ...
    network = model.dr_classifier_from_features(FLAGS.loss_type)
network.summary()
...
for epoch in range(n_epochs):
    ...
    for fnames, ex_features, he_features, ma_features, se_features, fundus_rescale_mean_subtract, grades in train_batch_fetcher():
        loss, acc = network.train_on_batch([ex_features, he_features,
                                            ma_features, se_features,
                                            fundus_rescale_mean_subtract], grades)
