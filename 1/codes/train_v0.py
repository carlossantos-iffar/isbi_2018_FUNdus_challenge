...
normalization_method = {"EX":"rescale", "HE":"rescale_mean_subtract", "MA":"rescale_mean_subtract", "SE":"rescale"}
if FLAGS.task == "SE":
    exclude_NAR = True
    NAR_subdir = "No_Apparent_Retinopathy_EX"
else:
    exclude_NAR = False
    NAR_subdir = "No_Apparent_Retinopathy_HE_MA_SE"
assert FLAGS.atrous_depth <= depths[FLAGS.task]
...
img_out_template = "/nfs/jaemin/isbi/1/segmentation_results/unet_01normalize/segmentation_results_{}/{}"
model_out_template = "/nfs/jaemin/isbi/1/models/unet_01normalize/model_{}/{}"
img_out_dir = img_out_template.format(FLAGS.atrous_depth, FLAGS.task + "_loss_weight_divisor_" + str(FLAGS.loss_weight_divisor))
model_out_dir = model_out_template.format(FLAGS.atrous_depth, FLAGS.task + "_loss_weight_divisor_" + str(FLAGS.loss_weight_divisor))
...
home_dir = "../data/preprocessed_640/"
AR = os.path.join(home_dir, "Apparent_Retinopathy")
NAR = os.path.join(home_dir, NAR_subdir)
...
train_pairs, val_pairs = utils.split_dataset(AR, NAR, seg_paths[FLAGS.task], 0.1, exclude_NAR=exclude_NAR)
train_batch_fetcher = iterator.BatchFetcher(train_pairs, FLAGS.batch_size, train_patch_size, True, normalization_method[FLAGS.task])
val_batch_fetcher = iterator.BatchFetcher(val_pairs, FLAGS.batch_size, validation_patch_size, False, normalization_method[FLAGS.task])
...
if FLAGS.load_model_dir:
    ...
    network = model.unet_atrous(depths[FLAGS.task], FLAGS.atrous_depth, FLAGS.loss_weight_divisor) 
#     network = model.atrous_conv_net(depths[FLAGS.task], FLAGS.loss_weight) 
network.summary()
