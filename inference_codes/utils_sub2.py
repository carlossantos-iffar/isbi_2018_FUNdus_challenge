...
def merge_csv_files():
    df_dr = pd.read_csv("outputs_sub2/dr.csv")
    df_dme = pd.read_csv("outputs_sub2/dme.csv")
    df = pd.merge(df_dme, df_dr, on="Image No", how="inner")
    
    df_new = df[["Image No", "DR Grade", "Risk of DME"]]
    df_new.to_csv("outputs_sub2/VRT_Disease_Grading.csv", index=False)
...
def resize_img(img, method):
    ...
    resized_img = imresize(padded, (process_len, process_len), method)
    
    return np.expand_dims(resized_img, axis=0)


def resize_img_dr(img, method):
    width_range = x_offset, x_offset + cropped_w
    img = img[:, :, width_range[0]:width_range[1], :]
    _, img_h, img_w, _ = img.shape
    len_side = width_range[1] - width_range[0]
...
def print_confusion_matrix(true_grades, pred_grades, grade_type):
    ...
    pred_grades = outputs2labels(pred_grades, min_label, max_label)
    ...
    print "weighted kappa: {}".format(cohen_kappa_score(true_grades, pred_grades, weights="quadratic"))
...
def save_figs_for_region_seg_check(segmented, od_segmentation, fovea_loc, img_out_dir, fnames):
    assert len(segmented.shape) == 4 and len(od_segmentation.shape) == 4 
    y, x = np.ogrid[:process_len, :process_len]
    
    for index in range(segmented.shape[0]):
        fname = os.path.basename(fnames[index])
        dir_name = img_out_dir
        if not os.path.isdir(dir_name):
            os.makedirs(dir_name)
        EX_segmented = (segmented[index, ..., 0] * 255).astype(np.uint8)
        Image.fromarray(EX_segmented).save(os.path.join(dir_name, fname + "_EX_segmented.png"))
